...
const ArgumentGeneration = ({ question, setUserArguments, setStep }) => {
  ...
  const handleSubmit = () => {
    ...
    // 🔹 Ensure step transition happens only after arguments update
    setTimeout(() => {
      console.log("Moving to Step 4: Argument Display");
      setStep(6); // ✅ Explicitly setting step 4 to ensure transition
    }, 100);
  };
