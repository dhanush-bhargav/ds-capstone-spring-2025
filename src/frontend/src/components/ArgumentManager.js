...
const handleProceed = async () => {
    ...
    try {
        ...
        const newArguments = response.data.argument_ids.map((id, index) => ({
            id: id,
            text: argumentsPayload[index].argument,
            pro: argumentsPayload[index].yes_or_no === "YES",
            categoryId: null
        }));

        // Update the global state with the *new* arguments from the API.
        setAllArguments([...allArguments, ...newArguments]);
        setStep(4); // Go to the next step
